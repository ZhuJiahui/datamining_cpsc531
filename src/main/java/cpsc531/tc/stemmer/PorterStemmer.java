...
public class PorterStemmer implements IStemmer {
	...
	@Override
	public String stemWord(String s) throws Exception {
		// char[] w = new char[501];
		int start = 0, end = 0;
		int strLen = s.length();
		...
		for (int i = 0; i < strLen; i++) {
			if (Character.isLetter(s.charAt(strLen - i - 1))) {
				end = strLen - i;
				break;
			}
		}
		String stripStr = s.substring(start, end);
		strLen = stripStr.length();
		if (strLen > 0 && strLen < 500) {
			stemmer.add(stripStr.toLowerCase().toCharArray(), strLen);
			stemmer.stem();
			{
